...
export const AssignCrewModal = ({ jobId, onClose }: Props): JSX.Element => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.85)" }}
      ...
      <div className="w-full max-w-md bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-fg text-sm">เพิ่มทีมงานเข้างาน</h2>
          </div>
        ...
        <div className="px-5 pt-4 pb-2 flex-shrink-0 space-y-2">
          <div className="relative">
            ...
            <input autoFocus placeholder="ค้นหาชื่อ / ตำแหน่ง…" value={search} onChange={(e) => setSearch(e.target.value)}
              className="w-full h-9 pl-9 pr-3 rounded-lg bg-fg/[0.04] border border-fg/[0.08] text-sm text-fg placeholder-fg/20 focus:outline-none focus:border-brand/40" />
          </div>
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setTypeFilter(null)}
              className={`h-6 px-2.5 rounded-full text-[11px] font-semibold border transition-colors ${!typeFilter ? "bg-brand text-black border-brand" : "text-fg/60 border-fg/10 hover:border-fg/30"}`}>ทั้งหมด</button>
            {types.map((ty) => (
              <button key={ty} onClick={() => setTypeFilter(typeFilter === ty ? null : ty)}
                className={`h-6 px-2.5 rounded-full text-[11px] font-semibold border transition-colors ${typeFilter === ty ? "bg-brand text-black border-brand" : "text-fg/60 border-fg/10 hover:border-fg/30"}`}>{CREW_TYPE_LABEL[ty]}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-2">
          {isLoading && (
            <div className="flex items-center justify-center gap-2 py-12 text-fg/60"><Loader2 className="w-4 h-4 animate-spin" /><span className="text-sm">กำลังโหลด…</span></div>
          )}
          {!isLoading && filtered.map((m) => (
            <button key={m.id} onClick={() => { setError(null); assignMutation.mutate(m.id); }} disabled={assignMutation.isPending}
              ...
              <div className="w-8 h-8 rounded-full bg-brand/10 flex items-center justify-center text-xs font-bold text-brand/70 flex-shrink-0">{initialsOf(m.name)}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-fg/80 truncate">{m.name}</p>
                <p className="text-[10px] text-fg/60">{CREW_TYPE_LABEL[m.type]}{m.role ? ` · ${m.role}` : ""}</p>
              </div>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Users className="w-8 h-8 text-fg/40" />
              <p className="text-xs text-fg/60">{available.length === 0 ? "ทีมงานทั้งหมดถูก assign แล้ว — เพิ่มคนใหม่ได้ที่หน้าทีมงาน" : "ไม่พบทีมงาน"}</p>
            </div>
          )}
        </div>
