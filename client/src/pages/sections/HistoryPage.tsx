        ...
        <div className="col-span-8 space-y-4">
          ...
          <div className="bg-surface-1 border border-fg/[0.06] rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-fg/[0.06] flex items-center gap-2">
              ...
              <span className="font-bold text-brand text-xs tracking-widest uppercase">{t("activityLogLabel")}</span>
              <span className="ml-auto text-[10px] text-fg/60">{t("entriesCount", { count: filtered.length })}</span>
            </div>
            <div className="divide-y divide-fg/[0.04] max-h-[500px] overflow-y-auto">
              {filtered.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="flex items-start gap-3 px-4 py-3 hover:bg-fg/[0.02] transition-colors" data-testid={`activity-${entry.id}`}>
                    <div className={`mt-0.5 w-6 h-6 rounded flex items-center justify-center bg-fg/[0.04] flex-shrink-0`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-fg/40">{entry.time}</span>
                      </div>
                      <p className="text-xs text-fg/50 mt-0.5">{entry.detail}</p>
                      <span className="text-[10px] text-fg/60">{entry.person}</span>
                    </div>
                  </div>
                );
              })}
              {filtered.length === 0 && (
                <div className="p-8 text-center text-fg/60 text-sm">{t("noActivityMatching")}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="col-span-4 space-y-4">
          <div className="bg-surface-1 border border-fg/[0.06] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-brand">{t("utilizationByCategory")}</span>
            </div>
            {utilizationData.map((cat) => (
              <div key={cat.category} className="mb-2 last:mb-0" data-testid={`utilization-${cat.category.toLowerCase().replace(/\s+/g, "-")}`}>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-fg/50">{cat.category}</span>
                  <span className={`font-semibold ${cat.pct >= 70 ? "text-emerald-400" : cat.pct >= 50 ? "text-amber-400" : "text-fg/60"}`}>{cat.pct}%</span>
                </div>
                <div className="w-full h-1.5 bg-fg/[0.04] rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-500 ${cat.pct >= 70 ? "bg-emerald-400/60" : cat.pct >= 50 ? "bg-amber-400/60" : "bg-fg/20"}`} style={{ width: `${cat.pct}%` }} />
                </div>
              </div>
            ))}
          </div>
            ...
            {healthMetrics ? (
              <>
                <div className="flex justify-between text-xs mb-2">
                  <span className="text-fg/60">{t("overallHealth")}</span>
                  <span className={`font-semibold ${healthMetrics.overall >= 80 ? "text-emerald-400" : "text-amber-400"}`}>{healthMetrics.overall}%</span>
                </div>
                <div className="flex justify-between text-xs mb-2">
                  <span className="text-fg/60">{t("assetsOut")}</span>
                  <span className="font-semibold text-amber-400">{healthMetrics.atRisk}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-fg/60">{t("inMaintenanceLabel")}</span>
                  <span className="font-semibold text-fg/60">{healthMetrics.maintenanceInProgress}</span>
                </div>
              </>
            ) : (
              <p className="text-xs text-fg/60">{t("noDataYet")}</p>
            )}
          </div>
          ...
          <div className="bg-surface-1 border border-fg/[0.06] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-brand">{t("revenueTrend6m")}</span>
            </div>
            <div className="flex items-end gap-1.5 h-20">
              {revenueMonths.map((m) => (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-0.5">
                  <div className="w-full flex flex-col items-center gap-px">
                    <div className="w-full bg-brand/30 rounded-t" style={{ height: `${(m.revenue / maxRevenue) * 60}px` }} />
                    <div className="w-full bg-emerald-400/30 rounded-b" style={{ height: `${(m.profit / maxRevenue) * 60}px` }} />
                  </div>
                  <span className="text-[9px] text-fg/60">{m.month}</span>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-4 mt-2 text-[10px]">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-brand/30" />{t("revenueLegend")}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-emerald-400/30" />{t("profitLegend")}</span>
            </div>
            {revenueMonths.length === 0 && (
              <p className="text-xs text-fg/60 text-center py-2">{t("noRevenueDataYet")}</p>
            )}
          </div>
        </div>
      </div>
