...
export const AddMaintenanceLogModal = ({ onClose, onSubmit }: AddMaintenanceLogModalProps): JSX.Element => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-fg">{t("addMaintenanceLog.title")}</h2>
          </div>
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-9 pl-9 pr-3 rounded-lg bg-black/40 border border-fg/10 text-sm text-fg
                  placeholder-fg/60 focus:outline-none focus:border-brand/40 transition-colors"
              />
            </div>
            ...
            <div className="flex items-center justify-between px-0.5">
              ...
              {selectedUnitIds.size > 0 && (
                <button
                  ...
                  className="text-[10px] text-fg/60 hover:text-red-400 transition-colors"
                >
                  {tc("clearAll")}
                </button>
              )}
            ...
            <div className="max-h-[260px] overflow-y-auto rounded-lg border border-fg/10 bg-black/20 p-2 space-y-1.5">
              {stockLoading && (
                <div className="flex items-center justify-center gap-2 py-8 text-fg/60">
                  ...
                  <div key={category}>
                    ...
                    >
                      {isFiltering
                        ...
                        : catOpen
                          ? <ChevronDown className="w-3.5 h-3.5 text-brand/60 flex-shrink-0" />
                          : <ChevronRight className="w-3.5 h-3.5 text-fg/60 flex-shrink-0" />
                      }
                      <span className="text-[11px] font-bold text-brand uppercase tracking-wider">{category}</span>
                      <span className="text-[10px] text-fg/60">{t("manageContainerUnits.modelsUnitsCount", { models: groups.length, units: catUnits })}</span>
                      {catSelected > 0 && (
                        <span className="ml-auto text-[10px] font-bold text-brand/60 px-1.5 py-0.5 rounded bg-brand/10">
                    ...
                    {catOpen && (
                      <div className="space-y-1.5">
                        {groups.map((group) => {
                          ...
                          return (
                            <div key={group.id} className="rounded-lg border border-fg/[0.06] overflow-hidden">
                              <div
                              ...
                              >
                                ...
                                >
                                  ...
                                  {!allSelected && selectedInGroup > 0 && (
                                    <div className="w-1.5 h-0.5 bg-brand rounded-full" />
                                  )}
                                </div>

                                <div className="flex-1 min-w-0">
                                  <p className="text-xs font-semibold text-fg/80 truncate">{group.name}</p>
                              ...
                              {isExpanded && groupUnits.map((unit) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex items-center gap-2.5 pl-8 pr-2.5 py-1.5 cursor-pointer border-t border-fg/[0.04] transition-colors
              ...
              {!stockLoading && filteredGroups.length === 0 && (
                <div className="flex flex-col items-center gap-2 py-8 text-center">
                  <Package className="w-6 h-6 text-fg/40" />
                  <p className="text-xs text-fg/60">{t("manageContainerUnits.noItemsFound")}</p>
                </div>
              )}
            </div>

            <p className="text-[10px] text-fg/40">{t("addMaintenanceLog.itemsToRepairHint")}</p>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              className="w-full bg-black/40 border border-fg/10 rounded-lg text-sm text-fg px-3 py-2 placeholder:text-fg/60 focus:outline-none focus:border-brand/40 transition-colors resize-none"
            />
          </div>
