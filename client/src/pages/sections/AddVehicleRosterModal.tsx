      ...
      <div className="w-full max-w-md bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06] flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-brand/10 flex items-center justify-center"><Truck className="w-4 h-4 text-brand" /></div>
            <h2 className="font-bold text-fg text-sm">{vehicle ? "แก้ไขรถ" : "เพิ่มรถ"}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          <div>
            <label className="block text-[11px] font-bold text-fg/70 mb-1">ชื่อรถ *</label>
          ...
          <div>
            <label className="block text-[11px] font-bold text-fg/70 mb-1">ประเภท</label>
            <input value={type} onChange={(e) => setType(e.target.value)} placeholder="เช่น รถ 6 ล้อ"
              className="w-full h-9 px-3 rounded-lg bg-fg/[0.04] border border-fg/10 text-sm text-fg placeholder:text-fg/30 focus:outline-none focus:border-brand/50" />
            <div className="flex flex-wrap gap-1.5 mt-1.5">
              {QUICK_TYPES.map((q) => (
                <button key={q} onClick={() => setType(q)}
                  className="h-6 px-2 rounded-full text-[10px] font-medium border border-fg/10 text-fg/50 hover:border-brand/40 hover:text-brand transition-colors">{q}</button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-bold text-fg/70 mb-1">ทะเบียน</label>
              <input value={plate} onChange={(e) => setPlate(e.target.value)} placeholder="1กก 1234"
                className="w-full h-9 px-3 rounded-lg bg-fg/[0.04] border border-fg/10 text-sm text-fg placeholder:text-fg/30 focus:outline-none focus:border-brand/50" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-fg/70 mb-1">ความจุ</label>
