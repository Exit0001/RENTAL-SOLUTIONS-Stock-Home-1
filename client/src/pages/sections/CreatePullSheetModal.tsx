      ...
      <div className="w-full max-w-lg bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-fg">{t("createPullSheet.title")}</h2>
          </div>
        ...
        <div className="p-5 flex flex-col gap-4">
          {fixedJobId ? (
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] text-fg/60 uppercase tracking-wider font-medium">{t("createPullSheet.jobLabel")}</label>
              <div className="w-full h-9 bg-black/40 border border-fg/10 rounded-lg text-sm text-fg/70 px-3 flex items-center truncate">
                {fixedJobName ?? "—"}
              </div>
            </div>
          ) : (
