...
export const DisposeModal = ({ onClose }: Props): JSX.Element => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.85)" }}
      ...
      <div className="w-full max-w-lg bg-[#0f0f0f] border border-white/[0.08] rounded-2xl shadow-2xl animate-modal-up flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-white text-sm">ขาย / ตัดของออกจากสต็อก</h2>
          </div>
            ...
            <>
              <div className="relative">
                ...
                <input autoFocus placeholder="ค้นหาอุปกรณ์ / serial…" value={search} onChange={(e) => setSearch(e.target.value)}
                  className="w-full h-9 pl-9 pr-3 rounded-lg bg-white/[0.04] border border-white/10 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#FFFF00]/40" />
              </div>
              <div className="space-y-1 max-h-72 overflow-y-auto">
                {isLoading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-white/40" /></div>
                  : filtered.slice(0, 60).map((it) => {
                    ...
                    return (
                      <button key={it.id} onClick={() => selectItem(it)}
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-lg border border-white/[0.06] bg-white/[0.02] hover:border-[#FFFF00]/30 hover:bg-white/[0.04] text-left transition-colors">
                        {bulk && <Layers className="w-3.5 h-3.5 text-amber-400/70 flex-shrink-0" />}
                        <div className="flex-1 min-w-0"><p className="text-sm text-white/85 truncate">{it.name}</p><p className="text-[10px] text-white/40">{it.brand} · {it.category}</p></div>
                        <span className="text-[11px] text-white/50 flex-shrink-0">ว่าง {avail}{bulk ? " (นับจำนวน)" : ""}</span>
                      </button>
                    );
                  })}
                {!isLoading && filtered.length === 0 && <p className="text-center text-xs text-white/40 py-8">ไม่พบอุปกรณ์</p>}
              </div>
            </>
          ) : (
            <>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setItem(null)} className="text-[11px] text-white/50 hover:text-white">← เปลี่ยน</button>
              ...
              {isBulk ? (
                <div>
                  <label className="block text-[11px] font-bold text-white/70 mb-1">จำนวนที่ตัดออก (มี {maxBulk})</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-white/70 mb-1">เลือก unit ที่ตัดออก ({selectedUnits.size}/{availUnits.length})</label>
                  <div className="space-y-1 max-h-52 overflow-y-auto">
                    {availUnits.length === 0 ? <p className="text-xs text-white/40 py-2">ไม่มี unit ว่าง</p> : availUnits.map((u) => {
                      ...
                      return (
                        <button key={u.id} onClick={() => setSelectedUnits((prev) => { const n = new Set(prev); n.has(u.id) ? n.delete(u.id) : n.add(u.id); return n; })}
                          className={`w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-left transition-colors ${on ? "bg-red-500/[0.08] border border-red-500/25" : "border border-white/[0.06] hover:bg-white/[0.03]"}`}>
                          <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 ${on ? "bg-red-500" : "border border-white/20"}`}>{on && <Check className="w-2.5 h-2.5 text-white" strokeWidth={3} />}</div>
                          <span className="text-xs text-white/85 truncate flex-1">{u.name}</span>
                          {u.serialNumber && <span className="text-[10px] text-white/40 font-mono">{u.serialNumber}</span>}
                        </button>
                      );
                    })}
                  </div>
              ...
              {/* reason */}
              <div>
                <label className="block text-[11px] font-bold text-white/70 mb-1.5">เหตุผล</label>
                <div className="flex flex-wrap gap-1.5">
                  {REASONS.map((r) => (
                    <button key={r} onClick={() => setReason(r)}
                      className={`h-8 px-3 rounded-lg text-xs font-semibold border transition-colors ${reason === r ? "bg-red-500 text-white border-red-500" : "text-white/60 border-white/10 hover:border-white/30"}`}>{REASON_LABEL[r]}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-white/70 mb-1">ราคาขายรวม (บาท)</label>
        ...
        {item && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-white/[0.06] flex-shrink-0 gap-3">
            ...
            <button onClick={() => { setError(null); disposeMutation.mutate(); }} disabled={!canSubmit || disposeMutation.isPending}
              className="flex items-center gap-2 h-9 px-5 rounded-lg text-sm font-bold text-white bg-red-600 hover:bg-red-700 transition-colors disabled:opacity-40">
