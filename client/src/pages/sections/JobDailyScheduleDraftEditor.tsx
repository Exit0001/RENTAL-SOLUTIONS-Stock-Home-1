...
export const JobDailyScheduleDraftEditor = ({ startDate, endDate, crew, schedules, dayCrew, onScheduleChange, onDayCrewChange }: Props): JSX.Element => {
  ...
  if (days.length === 0) {
    return <p className="text-xs text-white/40 italic">{t("dayScheduleNeedDates")}</p>;
  }
      ...
      {days.map((date) => {
        ...
        return (
          <div key={date} className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
            <button type="button" onClick={() => toggleExpand(date)} className="w-full flex items-center gap-3 px-3 py-2 hover:bg-white/[0.02] transition-colors text-left">
              <span className="text-xs font-semibold text-white/85 flex-shrink-0">{dateLabel}</span>
              ...
              {workingToday.length > 0 && (
                <span className="text-[10px] text-[#FFFF00]/70 flex items-center gap-1 flex-shrink-0">
                        ...
                        return (
                          <div key={c.crewMemberId} className="flex items-center gap-2">
                            <button
                              ...
                              className={`w-4 h-4 rounded border flex-shrink-0 transition-colors ${isChecked ? "bg-[#FFFF00] border-[#FFFF00]" : "border-white/20 hover:border-white/40"}`}
                            />
                            ...
                            {isChecked && (
                              <input
                                ...
                                className="h-6 w-32 px-2 text-[11px] rounded bg-black/30 border border-white/10 text-white/80 focus:outline-none focus:border-[#FFFF00]/40"
                              />
                            )}
                          </div>
                        );
