...
const EVENT_COLOR: Record<JobUnitEvent["eventType"], string> = {
  ...
  returned:   "text-brand bg-brand/10",
};
...
export const JobUnitEventsSection = ({ jobId, startDate, endDate }: Props): JSX.Element => {
  ...
  if (isLoading) return <Loader2 className="w-4 h-4 animate-spin text-fg/40" />;
  if (events.length === 0) return <p className="text-xs text-fg/40 italic">{t("noUnitEventsYet")}</p>;
  ...
  return (
    <div className="space-y-1 max-h-72 overflow-y-auto">
      {events.map((ev) => {
        ...
        return (
          <div key={ev.id} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-fg/[0.02] transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg/80 truncate">
                ...
                {ev.serialNumber && <span className="text-fg/40 font-mono ml-1.5">SN:{ev.serialNumber}</span>}
              </p>
