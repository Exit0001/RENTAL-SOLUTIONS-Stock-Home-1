  ...
  return (
    ...
    >
      <div className="w-full max-w-lg max-h-[85vh] flex flex-col bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl overflow-hidden animate-modal-up">
        ...
        {/* Header */}
        <div className="flex items-start justify-between px-5 py-4 border-b border-fg/[0.06] flex-shrink-0">
          <div className="flex-1 min-w-0 pr-2">
            <p className="text-[10px] text-fg/40 truncate">{itemName}</p>
            <div className="flex items-center gap-2 mt-0.5">
              <h3 className="font-bold text-fg text-sm truncate">{unit.name}</h3>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 pt-3 pb-1">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[9px] font-bold text-brand/40 uppercase tracking-widest">{t("sectionGeneral")}</span>
            </div>
            ...
            {unit.containerName && (
              <div className="flex items-center justify-between gap-3 py-1.5 border-b border-fg/[0.04] last:border-0">
                <span className="text-[10px] text-fg/60 flex-shrink-0 min-w-[90px]">{t("colContainer", { defaultValue: "Container" })}</span>
                <span className="flex items-center gap-1 text-xs text-fg/70">
                  <Layers className="w-3 h-3 text-brand/50" />{unit.containerName}
                </span>
              </div>
            )}
            {unit.healthScore != null && (
              <div className="flex items-center justify-between gap-3 py-1.5">
                <span className="text-[10px] text-fg/60 flex-shrink-0 min-w-[90px]">{t("healthScoreLabel", { defaultValue: "Health Score" })}</span>
          ...
          <div className="px-4 pt-3 pb-1">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[9px] font-bold text-brand/40 uppercase tracking-widest">{t("purchaseWarrantyLabel", { defaultValue: "Purchase / Warranty" })}</span>
            </div>
            <Row label={t("purchasedOnLabel", { defaultValue: "Purchased" })} value={fmtDate((unit as any).purchasedAt)} />
            <div className="flex items-start justify-between gap-3 py-1.5">
              <span className="text-[10px] text-fg/60 flex-shrink-0 min-w-[90px]">{t("warrantyExpLabel", { defaultValue: "Warranty Exp." })}</span>
              {fmtDate((unit as any).warrantyExpiresAt)
                ? <span className={`text-xs ${warrantyExpired ? "text-red-400 font-semibold" : "text-fg/70"}`}>
          ...
          <div className="px-4 pt-3 pb-4">
            <div className="flex items-center gap-1.5 mb-2">
              <Wrench className="w-3 h-3 text-brand/40" />
              <span className="text-[9px] font-bold text-brand/40 uppercase tracking-widest">{t("maintenanceHistoryLabel", { defaultValue: "Maintenance History" })}</span>
            </div>
            {logsLoading ? (
              ...
              <p className="text-[10px] text-fg/40 italic py-1">{t("noMaintenanceHistory", { defaultValue: "No maintenance history yet" })}</p>
            ) : (
              <div className="space-y-2">
                {logs.map((log) => {
                  const mc = maintenanceStatusColor[log.status] ?? maintenanceStatusColor.in_progress;
                  return (
                    <div key={log.id} className="rounded-lg border border-fg/[0.06] bg-fg/[0.02] p-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[10px] font-bold text-brand/70 uppercase">{tc(`statusEnum.${log.type}`, { defaultValue: log.type })}</span>
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          <span className="text-[10px] text-fg/50">{fmtDate(log.date)}</span>
                        </div>
                      </div>
                      <p className="text-xs text-fg/70 mt-1">{log.description}</p>
                      {log.cost && (
                        <p className="text-[10px] text-fg/40 mt-1">฿{Number(log.cost).toLocaleString()}</p>
                      )}
                    </div>
                  );
                })}
