        ...
        {groups.map((group) => (
          <div key={group.stockItemId} className="rounded-xl border border-fg/[0.06] bg-fg/[0.02] overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-fg/[0.04]">
              ...
              <span className="text-[10px] text-fg/40 flex-shrink-0">{group.unitIds.length}</span>
            </div>

            {group.unitIds.map((unitId) => {
              const info = unitInfo.get(unitId);
              return (
                <div key={unitId} className="flex items-center gap-2 px-3 py-1.5 border-t border-fg/[0.04]">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-fg/70 truncate">{info?.name ?? unitId}</p>
