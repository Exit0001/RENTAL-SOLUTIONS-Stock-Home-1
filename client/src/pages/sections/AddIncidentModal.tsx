      ...
      <div className="w-full max-w-lg bg-[#0f0f0f] border border-white/[0.08] rounded-2xl shadow-2xl animate-modal-up max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-white">Report Incident</h2>
          </div>
        ...
        <div className="p-5 flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <SelectField label="Job" value={jobId} onChange={setJobId}
          ...
          <div className="grid grid-cols-2 gap-3">
            <SelectField label="Stock Item (optional)" value={stockItemId}
              onChange={(v) => { setStockItemId(v); setStockUnitId(""); }}
              options={stockItems.map((i: any) => ({ value: i.id, label: i.itemName ?? i.name ?? i.id }))} />
            <SelectField label="Stock Unit (optional)" value={stockUnitId} onChange={setStockUnitId}
          ...
          {companyId && (
            <FileUploadField label="Photo Evidence" folder="incidents" companyId={companyId}
