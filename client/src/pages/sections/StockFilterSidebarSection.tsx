  ...
  return (
    <div>
      ...
      >
        ...
        {isLoading && <Loader2 className="w-3 h-3 text-white/20 animate-spin" />}
        ...
        {!isLoading && (
          <span className="ml-auto text-white/20 text-[10px]">{items.length}</span>
        )}
        <ChevronDown
        ...
        <>
          ...
          {showSearch && !isLoading && items.length > 8 && (
            <div className="relative mx-2 mb-2">
              ...
              <input
                ...
                className="w-full h-7 bg-white/[0.04] border border-white/[0.08] rounded-md pl-6 pr-6 text-[11px] text-white/60 placeholder:text-white/20 focus:outline-none focus:border-[#FFFF00]/30 focus:text-white transition-colors"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
          ...
          <div className="flex flex-col gap-0.5">
            {isLoading ? (
              ...
              <p className="px-2 py-2 text-[11px] text-white/20 italic">No results</p>
            ) : (
              filtered.map((item) => (
                <CheckItem
                  key={item.id}
