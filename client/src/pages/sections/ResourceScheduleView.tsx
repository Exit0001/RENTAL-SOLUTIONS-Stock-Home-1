...
const diffDays = (a: Date, b: Date): number => Math.round((b.getTime() - a.getTime()) / 86_400_000);

export type ResourceRow = { id: string; name: string; initials: string; sub?: string; group?: string };
interface Props {
  rows: ResourceRow[];
  jobs: any[];
  assignments: { resourceId: string; jobId: string }[];
  onBarClick?: (job: any) => void;
  emptyText?: string;
}

export function ResourceScheduleView({ rows, jobs, assignments, onBarClick, emptyText = "ยังไม่มีข้อมูล" }: Props) {
  const today = useMemo(() => startOfDay(new Date()), []);
  const gridRef = useRef<HTMLDivElement>(null);
  const VIEW_DAYS = useResponsiveDayCount(gridRef, COL_W, LEFT_W, MIN_DAYS, FALLBACK_DAYS);

  const [viewStart, setViewStart] = useState<Date>(() => {
    const d = startOfDay(new Date()); d.setDate(d.getDate() - 4); return d;
  });

  const days = useMemo(() =>
    Array.from({ length: VIEW_DAYS }, (_, i) => { const d = new Date(viewStart); d.setDate(d.getDate() + i); return d; }),
    [viewStart, VIEW_DAYS]);

  const resourceJobMap = useMemo(() => {
    const m = new Map<string, Set<string>>();
    for (const a of assignments) {
      if (!m.has(a.resourceId)) m.set(a.resourceId, new Set());
      m.get(a.resourceId)!.add(a.jobId);
    }
    return m;
  }, [assignments]);

  const jobById = useMemo(() => { const m = new Map<string, any>(); for (const j of jobs) m.set(j.id, j); return m; }, [jobs]);

  const todayOffset = diffDays(viewStart, today);
  const navigate = (weeks: number) => setViewStart((prev) => { const d = new Date(prev); d.setDate(d.getDate() + weeks * 7); return d; });
  const goToday = () => { const d = startOfDay(new Date()); d.setDate(d.getDate() - 4); setViewStart(d); };
    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#0a0a0a]">
      ...
      <div ref={gridRef} className="flex-1 overflow-auto">
        <div style={{ minWidth: totalW }}>
          ...
          {/* Resource rows */}
          {rows.map((c, ri) => {
            const showGroup = c.group && (ri === 0 || rows[ri - 1].group !== c.group);
            const jobIds = resourceJobMap.get(c.id) ?? new Set<string>();
            const resourceJobs = Array.from(jobIds).map((id) => jobById.get(id)).filter(Boolean);
