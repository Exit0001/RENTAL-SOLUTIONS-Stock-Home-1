...
export const AddCrewMemberModal = ({ member, onClose }: Props): JSX.Element => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.85)" }}
      ...
      <div className="w-full max-w-md bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-fg text-sm">{member ? "แก้ไขทีมงาน" : "เพิ่มทีมงาน"}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {/* Type pills */}
          <div>
            <label className="block text-[11px] font-bold text-fg/70 mb-1.5">ประเภท</label>
            <div className="flex flex-wrap gap-1.5">
              {TYPES.map((ty) => (
                <button key={ty} onClick={() => setType(ty)}
                  className={`h-8 px-3 rounded-lg text-xs font-semibold border transition-colors ${type === ty ? "bg-brand text-black border-brand" : "text-fg/60 border-fg/10 hover:border-fg/30"}`}>{CREW_TYPE_LABEL[ty]}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            <label className="block text-[11px] font-bold text-fg/70 mb-1">ชื่อ *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-bold text-fg/70 mb-1">เบอร์โทร</label>
              <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08x-xxx-xxxx"
                className="w-full h-9 px-3 rounded-lg bg-fg/[0.04] border border-fg/10 text-sm text-fg placeholder:text-fg/30 focus:outline-none focus:border-brand/50" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-fg/70 mb-1">ค่าตัว/วัน (บาท)</label>
          ...
          <div>
            <label className="block text-[11px] font-bold text-fg/70 mb-1">ตำแหน่ง</label>
          ...
          {type === "own_crew" && (
            <div>
              <label className="block text-[11px] font-bold text-fg/70 mb-1">ผูกกับบัญชีผู้ใช้ (optional)</label>
              ...
              <p className="text-[10px] text-fg/40 mt-1">ผูกบัญชีเพื่อให้คนนี้ได้รับแจ้งเตือนเมื่อถูก assign เข้างาน</p>
            </div>
          )}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-fg/[0.06] flex-shrink-0 gap-3">
          ...
          <button onClick={handleSave} disabled={saving}
            className="flex items-center gap-2 h-9 px-5 rounded-lg text-sm font-bold text-black transition-opacity hover:opacity-80 disabled:opacity-40" style={{ backgroundColor: "var(--brand)" }}>
