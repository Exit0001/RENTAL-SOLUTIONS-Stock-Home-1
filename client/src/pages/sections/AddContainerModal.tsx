      ...
      <div className="w-full max-w-md bg-surface-1 border border-fg/[0.08] rounded-2xl shadow-2xl animate-modal-up">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-fg/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-fg">{t("addContainer.title")}</h2>
          </div>
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              ...
              >
                {containerTypes.length === 0 && <option value="" className="bg-surface-1">{t("addContainer.noTypesYet")}</option>}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] text-fg/60 uppercase tracking-wider font-medium">
              {t("addContainer.quantityLabel")} <span className="text-fg/60 normal-case">{t("addContainer.quantityHint")}</span>
            </label>
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                className="w-9 h-9 rounded-lg border border-fg/10 flex items-center justify-center text-fg/60 hover:text-fg hover:border-fg/30 transition-colors flex-shrink-0"
              >
              ...
              <input
                ...
                className="w-16 h-9 bg-black/40 border border-fg/10 rounded-lg text-sm text-fg text-center focus:outline-none focus:border-brand/40 transition-colors"
              />
              <button
                type="button"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              {locations.length === 0 && <option value="" className="bg-surface-1">{t("addContainer.noLocationsYet")}</option>}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] text-fg/60 uppercase tracking-wider font-medium">
              {t("addContainer.barcodeLabel")} <span className="text-fg/60 normal-case">{t("addContainer.autoGeneratedHint")}</span>
            </label>
            <input
              type="text"
              value={barcode}
